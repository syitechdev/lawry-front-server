...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    // 👉 Bas-droite par défaut (mobile et desktop)
    className={cn(
      "fixed bottom-0 right-0 z-[9999] flex max-h-screen w-full max-w-full flex-col p-4 sm:max-w-[420px]",
      className
    )}
    {...props}
  />
    ...
    "data-[state=open]:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        warning:
          "group border-yellow-300 bg-yellow-50 text-yellow-900 " +
          "dark:border-yellow-700 dark:bg-yellow-950/50 dark:text-yellow-100",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium ring-offset-background transition-colors hover:bg-secondary " +
