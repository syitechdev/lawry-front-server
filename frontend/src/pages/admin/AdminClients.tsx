...
import UserServicesModal from "@/components/modals/UserServicesModal";
import { getUserServicesSummary } from "@/services/userServices";
...
const AdminClients: React.FC = () => {
  ...
  const [servicesModalUser, setServicesModalUser] = useState<User | null>(null);

  // Verrou suppression si usage de services
  const [userHasUsage, setUserHasUsage] = useState<Record<number, boolean>>({});
  ...
  // Charger les summaries pour griser le bouton supprimer si total > 0
  useEffect(() => {
    const run = async () => {
      ...
      for (const u of items) {
        try {
          const s = await getUserServicesSummary(u.id);
          entries.push([u.id, (s.total ?? 0) > 0]);
        } catch {
          entries.push([u.id, false]);
        }
      }
      setUserHasUsage(Object.fromEntries(entries));
    };
    run();
  }, [items]);
  ...
  const onEditSave = async () => {
    ...
    try {
      ...
      const originalRole: UserRole = (((original.roles || [])[0] as UserRole) ??
        "Client") as UserRole;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-red-50/30 to-gray-50">
                        ...
                        <td className="p-3">
                          <div className="flex flex-wrap gap-1">
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              title="Services du client"
                              onClick={() => {
                                setServicesModalUser(u);
                            ...
                            <Button
                              ...
                              title={
                                userHasUsage[u.id]
                                  ? "Ce client a utilisé des services — suppression interdite"
                                  : "Supprimer"
                              }
                              onClick={() => onDelete(u)}
      ...
      {servicesModalUser && (
        <UserServicesModal
          ...
          userId={servicesModalUser.id}
          userName={servicesModalUser.name}
        />
      )}
    </div>
  );
