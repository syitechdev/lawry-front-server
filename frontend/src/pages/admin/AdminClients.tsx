...
import { Eye, EyeOff, Edit, UserPlus, Filter, Download, Trash2 } from "lucide-react";
import BackofficeSidebar from "@/components/BackofficeSidebar";
import { useToast } from "@/hooks/use-toast";
import { users, type User } from "@/services/users";
import type { ListParams } from "@/services/crud";

type UserStatus = "Actif" | "VIP" | "Inactif";
...
type EditableUser = Pick<User, "name" | "email" | "phone" | "address"> & {
  status: UserStatus;
  password?: string;
...
function firstRole(u?: User): UserRole | undefined {
  const r = (u?.roles || [])[0];
...
const AdminClients: React.FC = () => {
  ...
  const success = (description: string, title = "Succès") => toast({ title, description });
  ...
  const onEditOpen = (u: User) => {
    ...
    setEditForm({
      ...
      phone: u.phone || "",
      address: u.address || "",
      status: (u.status as UserStatus) || "Actif",
      password: "",
      password_confirmation: "",
  ...
  const onEditSave = async () => {
    ...
    if (changed(editForm.name, original.name)) payload.name = trimOrUndefined(editForm.name);
    if (changed(editForm.email, original.email)) payload.email = trimOrUndefined(editForm.email);
    if (changed(editForm.status, original.status)) payload.status = editForm.status;
    if (changed(editForm.phone, original.phone)) payload.phone = trimOrNull(editForm.phone);
    if (changed(editForm.address, original.address)) payload.address = trimOrNull(editForm.address);
    ...
    try {
      ...
      const newRole: UserRole = (editForm.role ?? "Client") as UserRole;
      const originalRole: UserRole = (((original.roles || [])[0] as UserRole) ?? "Client") as UserRole;
      ...
      const anyE = e as any;
      const msg =
        anyE?.response?.data?.errors?.phone?.[0] ||
        anyE?.response?.data?.errors?.email?.[0] ||
  ...
  const onDelete = async (u: User) => {
    ...
    try {
      ...
      console.error(e);
      const anyE = e as any;
      errorToast(anyE?.response?.data?.message || "Suppression impossible");
    }
  };
  ...
  const onNewCreate = async () => {
    ...
    try {
      ...
      const targetRole: UserRole = (newUser.role ?? "Client") as UserRole;
  ...
  const exportCsv = () => {
    const rows = [
      ["Code/ID", "Nom", "Email", "Téléphone", "Statut", "Rôle", "Services", "Dernière activité", "Inscription"],
      ...filtered.map((u) => [
        u.code || u.id,
        u.name || "",
        u.email || "",
      ...
      <div className="ml-80 px-8 py-8">
        <div className="mb-8">
          <div className="bg-gradient-to-r from-red-900 to-red-800 text-white rounded-2xl p-6 shadow-xl">
            ...
            <p className="text-red-100">Base de données clients et suivi commercial</p>
          </div>
        </div>
          ...
          <CardHeader>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                ...
                <Button className="bg-red-900 hover:bg-red-800 w-full sm:w-auto" onClick={onNewOpen}>
                  ...
                  {!loading &&
                    pageItems.map((u) => (
                      <tr key={u.id} className="border-b hover:bg-gray-50">
                        <td className="p-3 font-mono text-sm">{u.code || u.id}</td>
                        <td className="p-3 font-medium">{u.name}</td>
                        <td className="p-3">{u.email}</td>
                        <td className="p-3">{u.phone || "—"}</td>
                        <td className="p-3">
                          <Badge className={statusBadge(u.status)}>{u.status || "Inactif"}</Badge>
                        </td>
                        <td className="p-3">
                          {u.last_activity_at ? new Date(u.last_activity_at).toLocaleDateString() : "—"}
                        </td>
                        <td className="p-3">
                          <div className="flex space-x-1">
                            <Button variant="ghost" size="sm" title="Voir" onClick={() => onView(u)}>
                              <Eye className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="sm" title="Modifier" onClick={() => onEditOpen(u)}>
            ...
            {!loading && filtered.length > 0 && (
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-4">
                <div className="text-sm text-gray-600">
                  {total === 0 ? "0 résultat" : `${firstIndex}–${lastIndex} sur ${total}`}
                </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-mono">{viewingClient.code || viewingClient.id}</p>
                </div>
                <div>
                  <Label>Statut</Label>
                  <Badge className={statusBadge(viewingClient.status)}>{viewingClient.status}</Badge>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p>{viewingClient.created_at ? new Date(viewingClient.created_at).toLocaleDateString() : "—"}</p>
                </div>
                <div>
                  <Label>Dernière activité</Label>
                  <p>
                    {viewingClient.last_activity_at
                      ? new Date(viewingClient.last_activity_at).toLocaleDateString()
                      : "—"}
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-bold text-blue-600">{viewingClient.services_count ?? 0}</p>
                </div>
                <div>
                  <Label>Rôle</Label>
        ...
        <Dialog open={!!editingClient} onOpenChange={() => setEditingClient(null)}>
          <DialogContent className="max-w-2xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setEditForm((s) => ({ ...s, status: e.target.value as UserStatus }))}
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      {showEditPwd2 ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </button>
                  </div>
                </div>
              </div>
              <p className="text-xs text-gray-500 -mt-2">Si vous laissez vide, le mot de passe actuel est conservé.</p>
            </div>
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button className="bg-red-900 hover:bg-red-800" onClick={onEditSave}>
                Sauvegarder
              </Button>
            </div>
          </DialogContent>
      ...
      <Dialog open={isNewClientDialogOpen} onOpenChange={setIsNewClientDialogOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="new-nom" value={newUser.name} onChange={(e) => setNewUser((s) => ({ ...s, name: e.target.value }))} />
              </div>
              <div>
                <Label htmlFor="new-email">Email</Label>
                <Input id="new-email" type="email" value={newUser.email} onChange={(e) => setNewUser((s) => ({ ...s, email: e.target.value }))} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="new-telephone" value={newUser.phone || ""} onChange={(e) => setNewUser((s) => ({ ...s, phone: e.target.value }))} />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewUser((s) => ({ ...s, status: e.target.value as UserStatus }))}
                >
                  <option value="Actif">Actif</option>
                  <option value="VIP">VIP</option>
                  <option value="Inactif">Inactif</option>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input id="new-address" value={newUser.address} onChange={(e) => setNewUser((s) => ({ ...s, address: e.target.value }))} />
              </div>
              <div />
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  <Input
                    ...
                    onChange={(e) => setNewUser((s) => ({ ...s, password_confirmation: e.target.value }))}
                  />
                  <button
                    type="button"
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-gray-500"
            ...
            <p className="text-xs text-gray-500 -mt-2">Laisser vide pour générer automatiquement un mot de passe côté serveur.</p>
          </div>
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button className="bg-red-900 hover:bg-red-800" onClick={onNewCreate}>
              Créer le Client
            </Button>
          </div>
        </DialogContent>
